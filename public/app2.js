...
function createTestCard(testId, config) {
  ...
  card.innerHTML = `
    ...
    <div style="margin-top:10px;display:flex;gap:8px;">
      <button type="button" class="run-one-btn run-btn" data-testid="${testId}" style="flex:1">▶ Run This Test</button>
      <button type="button" class="stop-one-btn remove-test-btn" data-testid="${testId}" style="flex:1;background:#f59e0b">⏹ Stop</button>
    </div>
  `;
...
function startTest(testId) {
  ...
  fetch(api.run, {
    ...
    eventSource.onerror = function(){
      ...
      testSources[testId] = null;
      resetButtons();
      console.log('Test #' + sseTestId + ' SSE disconnected');
      updateActiveTests();
    };
  }).catch(function(err){
    resetButtons();
...
function addTestToUI(config) {
  ...
  // Add default header after card is in DOM
  addHeaderRow(testId, 'Content-Type', 'application/json');
  return testId;
}

function initFloodgate() {
  console.log('initFloodgate() starting');
  checkK6();

  // Add one default test card
  addTestToUI();
...
function displayLiveMetrics(r) {
  ...
  var summaryHtml = 
    '<div class="metric-card"><div class="lbl">VUs</div><div class="val purple">' + (r.vus || r.config?.vus || 'N/A') + '</div></div>' +
  ...
  document.getElementById('latCards').innerHTML = latHtml;
}

function renderLatencyChart(r) {
  if (latChart) { latChart.destroy(); latChart = null; }
  var labels = ['p50', 'p90', 'p95', 'p99', 'max'];
  var values = [r.latency.p50, r.latency.p90, r.latency.p95, r.latency.p99, r.latency.max];
